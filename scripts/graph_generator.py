...
for i, row in enumerate(df.itertuples()):
    ax.text(-0.02, i-0.15, row.titolo, va='center', ha='right', fontfamily='DejaVu Sans Mono', fontsize=10, color='#a0f0d0', transform=ax.get_yaxis_transform())
    ax.text(-0.02, i+0.2, f"{row.autore} | {row.anno}", va='center', ha='right', fontfamily='DejaVu Sans Mono', fontsize=9, color='#c0e0b0', transform=ax.get_yaxis_transform())
...
for i, row in enumerate(df.itertuples()):
    if pd.notna(row.voto):
        ...
        ax.text(val + 0.1, i + 0.05, label, va='center', fontfamily='DejaVu Sans Mono', fontsize=10, color='#a0f0d0')
    else:
        ax.text(0.1, i, "Voto mancante", va='center', fontfamily='DejaVu Sans Mono', fontsize=10, color='red')
